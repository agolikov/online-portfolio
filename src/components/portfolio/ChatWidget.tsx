...
export function ChatWidget({ hash }: { hash: string }) {
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="fixed bottom-6 right-6 z-50 h-12 w-12 rounded-full bg-primary text-primary-foreground shadow-lg flex items-center justify-center hover:scale-105 active:scale-95 transition-transform print:hidden"
      ...
      {/* Panel */}
      {open && (
        <div className="fixed bottom-22 right-4 z-50 w-[min(24rem,calc(100vw-2rem))] flex flex-col rounded-2xl border border-border bg-background shadow-2xl overflow-hidden print:hidden"
          ...
          {/* Input */}
          <div className="border-t border-border px-3 py-2.5 flex gap-2 shrink-0 bg-background">
            <input
              ref={inputRef}
