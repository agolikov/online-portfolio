...
- [x] Alias-based resume pages — resumes can also be accessed by a user-defined alias `/:alias`; set and cleared from the edit page.
...
- [x] Project year field — side projects carry an optional `year` string; shown as a muted badge in the grid and `project · year` in PDF export (hidden when `hideYears` is set).
