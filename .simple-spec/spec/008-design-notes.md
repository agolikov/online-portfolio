...
- Portfolio section items use optional `enabled` flags for main-page visibility. Missing flags default to visible for backward compatibility; stories also honor the legacy `public` flag when `enabled` is absent.
...
- Cloudflare is the planned edge layer for public deployment: proxy DNS through Cloudflare, keep static assets cacheable, keep `/api/*` uncached, and rate-limit expensive API routes while leaving enabled `GET /api/resumes/:hash` anonymously readable.
