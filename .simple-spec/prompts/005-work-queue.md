...
Use this to pick a task from `.simple-spec/queue_tasks/` and execute it end-to-end.
...
1. List all task files in `.simple-spec/queue_tasks/` that have status `draft` or `refined`.
...
1. Read `.simple-spec/spec/05-architecture.md` and `.simple-spec/spec/02-stack.md` to understand current architecture and conventions.
...
1. Update every `.simple-spec/spec/` file affected by the implementation:
   - `.simple-spec/spec/03-features.md` — move features Planned → Implemented, or add new ones
   - `.simple-spec/spec/05-architecture.md` — update if module responsibilities changed
   - `.simple-spec/spec/07-dependencies.md` — add new packages
   - `.simple-spec/spec/06-data-models.md` — update if schema changed
   - `.simple-spec/spec/10-changelog.md` — add entry with today's date
...
5. Move the task file from `.simple-spec/queue_tasks/` to `.simple-spec/completed_tasks/` (keep the filename, update status to `done`).
